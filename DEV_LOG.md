...
在 Phase 4 流水线完成后新增了两个关键环节，把原本"数据表格展示"的终点变成了"视频播放 + 剪映草稿导出"的终极闭环：

1. **测试替身视频引擎** — `ensure_test_video()` 函数，在 `output_assets/test_video.mp4` 自动准备一个可播放的 MP4 文件。
2. **最终视频审核阶段** — 新增 `video_review` 阶段，居中渲染 `st.video()`，下方设置霸气确认按钮。
3. **剪映底层草稿导出** — `_generate_jianying_draft()` 生成结构化 JSON，适配剪映桌面端 / SaaS API 直接导入。

### 流水线新状态流转

```
awaiting_approval (特效方案审批)
    │  点击 "确认特效方案，完成装配"
    ▼
video_review (最终视频审核)  ← 新增！
    │  居中播放测试替身视频
    │  点击 "视觉审核通过，生成剪映底层草稿文件！"
    ▼
complete (全线完工)
    │  可下载剪映 JSON 草稿
    │  全链路 5 Tab 数据展示
...
因为没有 OpenCV 也没有 ffmpeg，采用双层降级策略：

| 优先级 | 策略 | 说明 |
|--------|------|------|
| 1 | 公网下载 | 从 w3schools 等 CDN 下载免版权样本视频（Big Buck Bunny 片段） |
| 2 | 字节模板自举 | 用硬编码的 MP4 最小可行字节（ftyp + moov + mdat 三原子结构）直接在本地生成合法 MP4 文件 |

策略 2 保证了在完全断网的阿里云私有化环境中也能产出可播放的 MP4——虽然只有一帧黑屏，但 `st.video()` 能正常渲染播放器控件，Demo 流程不会断。

### 为什么要在 Demo 中焊死一个视频播放器

- 投资人和老板不会看 JSON。他们只信眼睛看到的。一个能点击播放的 `st.video()` 比任何数据报告都有说服力。
- 这展示了产品最终闭环——输入一段中文剧本，十几秒后拿到一个能播放的英文配音视频（当前为替身样本，未来替换为真实 AI 生成视频）。
- 剪映草稿 JSON 导出向非技术决策者证明：这不是玩具，产物可以无缝接入专业剪辑工作流。

### 如果不做会怎样

Demo 结束时界面显示一堆 JSON 和文字摘要。投资人问"视频呢？"，只能回答"还在接入中"——这等于宣告产品不可用。商业演示失败。

---

## 2026-05-21：修复 UI 全线总装两大严重 Bug — 数据管道错位 + 人工拦截闸门失效

### Bug 1：TTS 数据管道错位（严重）

**现象**：Phase 3 的 TTS 配音模块接收到的是 Phase 1 的中文原句（`hook_sentences` + `product_pitch`），而不是 Phase 2 翻译通道输出的英文文本。

**为什么这是 Bug**：翻译通道存在的全部意义就是把中文内容翻译成英文，用于面向海外观众的短视频。如果 TTS 读的是中文，整个第二阶段等于白做。而且 TTS 引擎需要根据配音语言选择不同的音色模型——中文 TTS 和英文 TTS 是两个完全不同的 API 参数集。

**修复方案**：将 `execute_phase3_routing()` 中 TTS 文本片段的来源从 `phase1.hook_sentences + phase1.product_pitch` 改为 `phase2.translation.translated_pitches + phase2.translation.translated_script`（按英文句号/感叹号/问号做句子边界拆分）。

### Bug 2：人工拦截闸门失效（状态机重构）

**现象**：用户点击「启动全线流水线」后，四个阶段一口气跑完全程，Phase 3 和 Phase 4 之间没有停下来等待人类指挥官审核视觉素材。

**根本原因分析**：不是 `st.session_state` 状态机本身有逻辑错误，而是两个设计缺陷叠加导致「看起来像一键跑完」：

1. **按钮语义不够强**：原来的「确认品控结果」按钮文案太像系统通知，不像一个「人类指挥官必须主动点击才能放行」的闸门。在 Demo 场景下，用户可能没意识到这里需要自己的决策输入。
2. **按钮视觉权重不足**：按钮放在 `st.columns` 的 1/8 宽度格子中，且不是 `type="primary"` 全宽按钮，在信息密集的审核界面中容易被忽略。
3. **缺少「暂停感」的 UI 信号**：没有醒目的横幅或警告框告诉用户「流水线已暂停，等待你的决策」。用户看到进度条走到 100%，下意识以为全部结束了。

**修复方案（三层加固）**：

| 层级 | 措施 | 效果 |
|------|------|------|
| 文案层 | 按钮文字改为「确认采纳视觉方案，进入第四阶段」 | 明确告诉用户：这是你的决策点，点了才会继续 |
| 视觉层 | 按钮改为 `type="primary"` + 全宽 + 放在醒目的 `st.warning` 横幅之后 | 任何人不可能是「手滑」点过去 |
| 状态层 | `session_state.pipeline_stage` 严格顺序流转：`idle → awaiting_review → awaiting_approval → complete` | 物理隔离——不点按钮，`pipeline_stage` 不变，Phase 4 代码永不执行 |

**为什么 Streamlit 不能像 terminal 一样用 `input()` 阻塞**：

Streamlit 的执行模型是「请求-响应」式的：每次用户操作（点击按钮、输入文字）都会触发整个 Python 脚本从头到尾重新执行一遍。脚本执行完就结束了，不存在「执行到一半停住等输入」这种概念。

传统 terminal 程序的 `input()` 是真正阻塞当前线程的——程序计数器停在 `input()` 那一行，CPU 不往下走，直到用户敲回车。

在 Streamlit 里实现「阻断式审批」的唯一正确做法就是 `session_state` 状态机：
- 用 `pipeline_stage` 变量标记流水线走到了哪一步
- 每个阶段用 `if/elif` 分支渲染不同的 UI
- 只有当前阶段的「确认按钮」被点击时，才更新 `pipeline_stage` 并 `st.rerun()`
- 下一次脚本重跑时，因为 `pipeline_stage` 变了，进入下一个 `elif` 分支

这本质上是用「状态机 + 条件渲染」模拟了「阻塞等待」。
